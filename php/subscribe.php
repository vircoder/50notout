...
if(!empty($_POST)){
	...
	$stmt = $connect->prepare("INSERT INTO subscribers (name,email) VALUES (?, ?)");
$stmt->bind_param("ss", $name, $email);
	...
	mail($email,"50notout subscription","You have subscribed to receive email regarding 50notout newsletters and events.");
	
//	$insert = mysqli_query($connect,"insert into tblqueries(name,email,phone,description) values('$name','$email','$phone','$description')");
	if($insert){
		$sendstatus = 1;
		$message = '<div class="alert alert-success subscription-success" role="alert"><strong>Success!</strong> You have subscribed successfully.</div>';
	}else{
		$sendstatus = 0;
